...
def unpack_shows(show_tuples):
    return list(map(lambda show_tuple: {**show_tuple[0].__dict__, **show_tuple[1].__dict__}, show_tuples))
...
def show_venue(venue_id):
  ...
  past_shows = db.session.query(Show, Artist).join(Artist, Artist.id == Show.artist_id).filter(Show.venue_id == venue_id, Show.start_time <= datetime.now()).all() 
  # convert the resulting tuple into combined dictionaries of the resulting Show and Artist Objects
  past_shows = unpack_shows(past_shows)
  past_shows = datetimes_to_strings(past_shows)
  print(past_shows)

  upcoming_shows = db.session.query(Show, Artist).join(Artist, Artist.id == Show.artist_id).filter(Show.venue_id == venue_id, Show.start_time > datetime.now()).all()
  upcoming_shows = unpack_shows(upcoming_shows)
  upcoming_shows = datetimes_to_strings(upcoming_shows)
